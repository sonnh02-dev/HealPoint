...
export const doctorService = {
  async getDoctorList(pageIndex:number,itemsPerPage:number): Promise<IDoctor[]> {
    try {

      const res = await fetch(`${apiUrl}/api/doctors$?skip=${(pageIndex-1)*itemsPerPage}&top=${itemsPerPage}`, {
        method: 'GET',
        headers: {
  ...
  async getDoctorListByServiceId(serviceId: string | number): Promise<IDoctor[]> {
    try {
      ...
      const res = await fetch(`${apiUrl}/api/doctors/by-service/${serviceId}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
  ...
  async getDoctorsBySpecialtyId(specialtyId: string | number): Promise<IDoctor[]> {
    try {
      ...
      const res = await fetch(`${apiUrl}/api/doctors/by-specialty/${specialtyId}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
